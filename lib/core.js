...
async function createProject(name) {
  ...
  return execAsync(
    ...
    return Promise.all([
      ...
      fs.rm(path.resolve(projectDirectory, '.git'), {
        recursive: true,
        force: true,
      }),
      execAsync(`git -C ${projectDirectory} init`),
    ]).then(([packageJson]) => {
...
function generateUserScriptHeader(configuration) {
  let header = `// @name\t${configuration.name ?? ''}\n// @version\t${configuration.version ?? ''}\n// @description\t${configuration.description ?? ''}\n// @license\t${configuration.license ?? ''}\n`;
  const skipItems = ['name', 'version', 'description', 'license'];
  for (const key in configuration) {
    if (skipItems.includes(key)) continue;
