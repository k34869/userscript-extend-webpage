...
function gElementAndStyleCode(options) {
    ...
    elements.each((i, el) => {
        ...
        for (const [key, value] of Object.entries(attribs)) {
            if (key === 'append' && value !== '') {
                elsInsertDomCode += `$.getDOM(\`${value}\`).then(el => { el.append(options.__elements__.$${name}); });`;
            } else if (key === 'prepend' && value !== '') {
                elsInsertDomCode += `$.getDOM(\`${value}\`).then(el => { el.prepend(options.__elements__.$${name}); });`;
            } else if (key === 'replace' && value !== '') {
                elsInsertDomCode += `$.getDOM(\`${value}\`).then(el => { el.html(options.__elements__.$${name}); });`;
            }
        }
        elsCode += `$${name}: $(\`${html}\`),`
    })
...
async function modulePack(input) {
...
async function gResInjectCode(options) {
    const { routeName, resFiles, resFilesMap } = options;
    let requireCode = '';
    if (Array.isArray(resFiles)) {
        for (const e of resFiles) {
            if (e.search(/^@\//) === -1) {
                throw new Error(`'${req[key]}' must start with '@/'`);
            }
            if (e in resFilesMap) {
                requireCode += `'${e}': ${resFilesMap[e]}.require('${e}', { isCall: false, __exports__ }), `;
            } else {
                ...
                if (extName.search(/^jpg$|^jpeg$|^png$|^webp$|^bmp$|^gif$|^woff$|^woff2$/g) !== -1) {
                    ...
                    let jsCode
                    if (/\.prim.js$/.test(resPath)) {
                        jsCode = fs.readFileSync(resPath, 'utf8');
                    } else {
                        jsCode = await modulePack(resPath);
                    }
        ...
        return `${routeName}.require = (requireFile, { isCall = true, exports: __exports__ } = {}) => { const require = { ${requireCode} };const d = require[requireFile];return isCall ? d() : d; }`;
    } else {
        return '';
    }
}

async function gRouteExecCode(routeName, { routesPattern, routesExecutorCode, routesName, resFilesMap }) {
    ...
    const resInjectCode = await gResInjectCode({ routeName, resFiles, resFilesMap });
